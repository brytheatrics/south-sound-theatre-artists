...
- [ ] **Re-hide incomplete bulk-imported profiles.** Currently 21 profiles are temporarily published so the directory looks populated during staging review. Re-run `node scripts/gate-incomplete-profiles.mjs` (or the equivalent SQL) before invitations go out so they arrive at the "your profile isn't visible yet" banner state.
