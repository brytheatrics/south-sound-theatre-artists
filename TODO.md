...
- [ ] **`event_sources` notes for 26 orgs.** All in operator voice, but worth a scan in case any wording feels off. Live in `scripts/seed-event-sources.mjs`; surface in `/admin/calendar` per-source view.
