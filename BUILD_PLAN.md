...
Calendar marquee items now deep-link to `/calendar?highlight=<production_id>`.
On mount, the calendar page finds every `.perf-pill` (grid) + `.perf-row`
(list) tagged with that production, scrolls the first match into view
(only when not already on-screen), and pulses an `.is-highlighted` class
for 2 seconds. URL param sticks around, so a refresh re-triggers the pulse.
...
- **Weekly community digest (callboard + calendar).** Most of the plumbing already exists but there's no public path into it. The `callboard_subscriptions` table, `scripts/callboard-weekly-digest.mjs` cron, `.github/workflows/callboard-weekly-digest.yml`, and `callboard_weekly_digest` email template all ship today; the table just has zero rows because nobody can subscribe. Scope:
  - Add a `confirmed_at` column to `callboard_subscriptions` so we can do double opt-in (email confirmation before the subscription activates). Defends against someone signing up other people's emails.
  - Build a subscribe form component used on `/callboard` and `/calendar` (email + optional discipline filters; falls back to "everything" if none picked).
  - Server action: insert pending subscription, fire confirmation email with a single-use token.
  - New email template (small) for the confirmation step.
  - `/callboard/subscribe/confirm/[token]` route to flip `confirmed_at` and activate the row.
...
- **Calendar-sync title-drift checker.** The LLM extraction in `scripts/_lib/calendar-sync.mjs` occasionally normalizes proper nouns to more-common spellings ("Mamma Mia!" -> "Mama Mia" was the first caught instance, fixed in commit 713a8f7 by adding an explicit "preserve titles verbatim" rule to the season-extraction prompt). The prompt fix should prevent recurrences but doesn't catch them when they do happen. Build only if title drift surfaces a second time:
  - `scripts/check-calendar-titles.mjs` walks every approved production with an organization_id and re-fetches the source page (or pulls the cached source HTML if we add that to the schema), then runs a Levenshtein-distance check between the stored title and the closest token sequence in the source. Flag anything with distance > 2 OR length difference > 1.
  - Output: a CSV / markdown report listing `<production_id> | <stored title> | <suggested fix from source> | <admin/calendar/[id]/edit URL>` so admin can review + fix in one pass.
  - Runs on demand (no cron). Lexi or Blake invokes after a quarterly cron walk.
  - Estimated ~1 hour. Don't build until there's a second confirmed false positive - one-offs are cheaper to hand-fix.
